...
const { toWebsiteName, scale, useWheelEvent, showRankList, showWarpList, showVersionList, selectVersionList, showFactionList, selectFactionList} = storeToRefs(characterSettingStore)
const { WebsiteNameList, scaleMin, scaleMax, selectRankList, selectWarpList } = characterSettingStore
...
const CHAR_SETTING_FOLD_KEY = 'zzz-CharacterSettingFold'
//获取折叠缓存
const getCharacterSettingFold = () => {
    // 从缓存中读取名为 CHAR_SETTING_FOLD_KEY 的数据
    const tempCharacterSettingFold = localStorage.getItem(CHAR_SETTING_FOLD_KEY)

    // 检查是否存在名为 CHAR_SETTING_FOLD_KEY 的数据
...
const saveCharacterSettingFold = () => {
    // 将对象转换为字符串，并将其存储在缓存中
    localStorage.setItem(CHAR_SETTING_FOLD_KEY, JSON.stringify(hadFold.value))
}
...
<template>
    <div class="character-setting-fold">
        <div class="character-setting" id="character-setting" :class="hadFold ? 'nowarp' : 'warp'" @wheel="handleSettingScroll">
            ...
            <!-- <h1>代理人列表设置（滚轮控制事件的缩放基础增量（可选实现，或者和开关合并提供 关闭、0.05、0.1、0.15、0.2 四个选项））</h1> -->
            ...
            <!-- <div class="character-setting-selectbox"> -->
                <div class="character-setting-select-input">
                    ...
                    multiple
                    placeholder="显示所有稀有度代理人"
                    style="width: 202px;"
                    >
                        <el-option
                            v-for="item in selectRankList"
                            :key="item.value"
                            :label="item.label"
                            :value="item.value"
                        />
