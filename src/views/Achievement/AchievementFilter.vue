...
const ACHIEVEMENT_FILTER_FOLD_KEY = 'zzz-AchievementFilterFold'
//获取折叠缓存
const getAchievementFilterFold = () => {
    // 从缓存中读取名为 ACHIEVEMENT_FILTER_FOLD_KEY 的数据
    const tempAchievementFilterFold = localStorage.getItem(ACHIEVEMENT_FILTER_FOLD_KEY)

    // 检查是否存在名为 ACHIEVEMENT_FILTER_FOLD_KEY 的数据
...
const saveAchievementFilterFold = () => {
    // 将对象转换为字符串，并将其存储在缓存中
    localStorage.setItem(ACHIEVEMENT_FILTER_FOLD_KEY, JSON.stringify(hadFold.value))
}
